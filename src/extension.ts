...
export function activate(context: vscode.ExtensionContext) {
	...
	let generatelog = vscode.commands.registerCommand('extension.generatelog', () => {
		...
		<body>
			<input placeholder="腾讯文档链接" id="tencentDoclink" required />
			<input id="startTimeInput" required />
			<input id="endTimeInput" required />
			<p id="tencentLogLink"></p>
			<p id="tencentListLogLink"></p>
			<button id="clipboardCopy1">复制:捞取 Excel 的日志</button>
			<button id="clipboardCopy2">复制:捞取 Excel 列表页项目的日志</button>
			<script>
			...
			<script>
				const tencentLogLink = document.querySelector('#tencentLogLink');
				const tencentListLogLink = document.querySelector('#tencentListLogLink');
				const tencentDoclink = document.querySelector('#tencentDoclink');
				const startTimeInput = document.querySelector('#startTimeInput');
				const endTimeInput = document.querySelector('#endTimeInput');
				const clipboardCopy1 = document.querySelector('#clipboardCopy1');
		
				let startTimeStamp = new Date().getTime();
				let endTimeStamp = new Date().getTime();
		
				const convertLink = () => {
					const link1 = tencentDoclink.value + '&wl=1&wlPlugin=WL_REPORT_DIALOG&startTs=' + startTimeStamp + '&endTs=' + endTimeStamp;
					const link2 = 'https://docs.qq.com/desktop/?' + 'wl=1&wlPlugin=WL_REPORT_DIALOG&startTs=' + startTimeStamp + '&endTs=' + endTimeStamp;
					clipboardCopy1.setAttribute('data-clipboard-text', link1);
					clipboardCopy2.setAttribute('data-clipboard-text', link2);
					console.log(link1, link2);
					tencentLogLink.innerHTML = link1;
					tencentListLogLink.innerHTML = link2;
				}
				...
				tencentDoclink.onchange = (event) => {
					convertLink();
				};
		
				startTimeInput.value = timestampToTime(new Date().getTime());
				startTimeInput.onchange = (event) => {
					console.log(event.target.value);
					startTimeStamp = new Date(event.target.value).getTime();
					startTimeInput.value = event.target.value;
					convertLink();
				};
		
				endTimeInput.value = timestampToTime(new Date().getTime());
				endTimeInput.onchange = (event) => {
					console.log(event.target.value);
					endTimeStamp = new Date(event.target.value).getTime();
					endTimeInput.value = event.target.value;
					convertLink();
				};
		
				new ClipboardJS('#clipboardCopy1');
				new ClipboardJS('#clipboardCopy2');
			</script>
		</body>
